...
#define TAG "com.mobidonia.tistartapp"

using namespace v8;

static Persistent<Object> bindingCache;

static void Tistartapp_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Tistartapp.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::TistartappBindings::lookupGeneratedInit(
...
static void Tistartapp_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Tistartapp_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Tistartapp_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TistartappBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TistartappBinding = {
	"com.mobidonia.tistartapp",
	Tistartapp_init,
	Tistartapp_dispose
};
...
Java_com_mobidonia_tistartapp_TistartappBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.mobidonia.tistartapp", &TistartappBinding);
	titanium::KrollBindings::addExternalLookup(&(::TistartappBindings::lookupGeneratedInit));
}
