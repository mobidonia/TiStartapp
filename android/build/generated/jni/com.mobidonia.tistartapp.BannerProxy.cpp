...
BannerProxy::BannerProxy(jobject javaObject) : titanium::Proxy(javaObject)
{
}
...
Local<FunctionTemplate> BannerProxy::getProxyTemplate(Isolate* isolate)
{
	if (!proxyTemplate.IsEmpty()) {
	...
	javaClass = titanium::JNIUtil::findClass("com/mobidonia/tistartapp/BannerProxy");
...
, javaClass, nameSymbol);
	...
	t->Set(titanium::Proxy::inheritSymbol.Get(isolate),
		FunctionTemplate::New(isolate, titanium::Proxy::inherit<BannerProxy>)->GetFunction());

	titanium::ProxyFactory::registerProxyPair(javaClass, *t);

	// Method bindings --------------------------------------------------------
	titanium::SetProtoMethod(isolate, t, "printMessage", BannerProxy::printMessage);
	titanium::SetProtoMethod(isolate, t, "getMessage", BannerProxy::getMessage);
	titanium::SetProtoMethod(isolate, t, "setMessage", BannerProxy::setMessage);
	...
	instanceTemplate->SetAccessor(NEW_SYMBOL(isolate, "message"),
			BannerProxy::getter_message,
			BannerProxy::setter_message,
			Local<Value>(), DEFAULT,
...
// Methods --------------------------------------------------------------------
void BannerProxy::printMessage(const FunctionCallbackInfo<Value>& args)
{
	...
	if (!methodID) {
		methodID = env->GetMethodID(BannerProxy::javaClass, "printMessage", "(Ljava/lang/String;)V");
		if (!methodID) {
...
void BannerProxy::getMessage(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "getMessage()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(BannerProxy::javaClass, "getMessage", "()Ljava/lang/String;");
		if (!methodID) {
...
void BannerProxy::setMessage(const FunctionCallbackInfo<Value>& args)
{
	LOGD(TAG, "setMessage()");
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(BannerProxy::javaClass, "setMessage", "(Ljava/lang/String;)V");
		if (!methodID) {
...
void BannerProxy::getter_message(Local<Name> property, const PropertyCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
...
void BannerProxy::setter_message(Local<Name> property, Local<Value> value, const PropertyCallbackInfo<void>& args)
{
	Isolate* isolate = args.GetIsolate();
	HandleScope scope(isolate);
